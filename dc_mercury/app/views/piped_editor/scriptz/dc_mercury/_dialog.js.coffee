...
class @DcMercury.Dialog
  ...
  build: ->
    @element = jQuery('<div>', {class: "dc_mercury-dialog dc_mercury-#{@name}-dialog loading", style: 'display:none'})
  ...
  load: (callback) ->
    ...
    if DcMercury.preloadedViews[@url]
      ...
      @loadContent(DcMercury.preloadedViews[@url])
      ...
      DcMercury.dialogHandlers[@name].call(@) if DcMercury.dialogHandlers[@name]
      ...
      jQuery.ajax @url, {
        ...
        error: =>
          ...
          DcMercury.notify('DcMercury was unable to load %s for the "%s" dialog.', @url, @name)
      }
