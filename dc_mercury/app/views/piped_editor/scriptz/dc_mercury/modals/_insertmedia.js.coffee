@DcMercury.modalHandlers.insertMedia = ->
    ...
    switch type
      ...
      when 'youtube_url'
        ...
        unless /^http:\/\/youtu.be\//.test(url)
          DcMercury.notify('Error: The provided youtube share url was invalid.')
          return
        ...
        DcMercury.trigger('action', {action: 'insertHTML', value: value})

      when 'vimeo_url'
        url = @element.find('#media_vimeo_url').val()
        unless /^http:\/\/vimeo.com\//.test(url)
          DcMercury.notify('Error: The provided vimeo url was invalid.')
          return
