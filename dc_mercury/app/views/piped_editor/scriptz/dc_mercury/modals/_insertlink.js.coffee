@DcMercury.modalHandlers.insertLink = ->
  ...
  for link in jQuery('a[name]', window.dc_mercuryInstance.document)
  ...
  @element.find('form').on 'submit', (event) =>
    ...
    if existingLink
      DcMercury.trigger('action', {action: 'replaceLink', value: value, node: existingLink.get(0)})
    else
      DcMercury.trigger('action', {action: 'insertLink', value: value})
