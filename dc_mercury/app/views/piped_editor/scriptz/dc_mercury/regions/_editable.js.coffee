class @DcMercury.Regions.Editable extends DcMercury.Region
  ...
  build: ->
    ...
    for element in @element.find('.dc_mercury-snippet')
      element.contentEditable = false
      jQuery(element).attr('data-version', '1')

    # add the basic editor settings to the document (only once)
    unless @document.dc_mercuryEditing
      @document.dc_mercuryEditing = true
      try
  ...
  bindEvents: ->
    ...
    DcMercury.on 'region:update', =>
      ...
      if currentElement.length
        ...
        DcMercury.tableEditor(table, currentElement.closest('tr, td'), '<br/>') if table.length
        # display a tooltip if we're in an anchor
        anchor = currentElement.closest('a', @element)
        if anchor.length && anchor.attr('href')
          DcMercury.tooltip(anchor, "<a href=\"#{anchor.attr('href')}\" target=\"_blank\">#{anchor.attr('href')}</a>", {position: 'below'})
        else
          DcMercury.tooltip.hide()

    @element.on 'dragenter', (event) =>
      return if @previewing
      event.preventDefault() unless DcMercury.snippet
    ...
    @element.on 'drop', (event) =>
      ...
      @focus()
      DcMercury.uploader(event.originalEvent.dataTransfer.files[0])
    ...
    @element.on 'possible:drop', =>
      ...
      if snippet = @element.find('img[data-snippet]').get(0)
        @focus()
        DcMercury.Snippet.displayOptionsFor(jQuery(snippet).data('snippet'))
    ...
    @element.on 'dblclick', (event) =>
      ...
      if image.length
        ...
        DcMercury.trigger('button', {action: 'insertMedia'})

    @element.on 'mouseup', =>
      return if @previewing
      @pushHistory()
      DcMercury.trigger('region:update', {region: @})
  ...
  content: (value = null, filterSnippets = true, includeMarker = false) ->
    if value != null
      ...
      for element in container.find('[data-snippet]')
        ...
        if snippet = DcMercury.Snippet.find(element.data('snippet'))
  ...
  execCommand: (action, options = {}) ->
    ...
    if handler = DcMercury.config.behaviors[action] || DcMercury.Regions.Editable.actions[action]
    ...
    @element.find('img').one 'error', -> jQuery(@).attr({src: '/images/dc_mercury/missing-image.png', title: 'Image not found'})
  ...
  selection: ->
    return new DcMercury.Regions.Editable.Selection(@window.getSelection(), @document)
  ...
  handlePaste: (event) ->
    ...
    if DcMercury.config.pasting.sanitize == 'text' && event.clipboardData
      ...
      sanitizer = jQuery('#dc_mercury_sanitizer', @document).focus()
  ...
  sanitize: (sanitizer) ->
    ...
    sanitizer.find(".#{DcMercury.config.regions.className}").remove()

    if DcMercury.config.pasting.sanitize
      switch DcMercury.config.pasting.sanitize
        when 'blacklist'
        ...
        when 'whitelist'
          for element in sanitizer.find('*')
            allowed = false
            for allowedTag, allowedAttributes of DcMercury.config.pasting.whitelist
  ...
  @actions: {
    ...
    insertRowBefore: -> DcMercury.tableEditor.addRow('before')

    insertRowAfter: -> DcMercury.tableEditor.addRow('after')

    insertColumnBefore: -> DcMercury.tableEditor.addColumn('before')

    insertColumnAfter: -> DcMercury.tableEditor.addColumn('after')

    deleteColumn: -> DcMercury.tableEditor.removeColumn()

    deleteRow: -> DcMercury.tableEditor.removeRow()

    increaseColspan: -> DcMercury.tableEditor.increaseColspan()

    decreaseColspan: -> DcMercury.tableEditor.decreaseColspan()

    increaseRowspan: -> DcMercury.tableEditor.increaseRowspan()

    decreaseRowspan: -> DcMercury.tableEditor.decreaseRowspan()
...
class DcMercury.Regions.Editable.Selection
  ...
  forceSelection: (element) ->
    ...
    if @range
      if @commonAncestor(true).closest('.dc_mercury-snippet').length
  ...
  selectMarker: (context) ->
    markers = context.find('em.dc_mercury-marker')
  ...
  placeMarker: ->
    ...
    @startMarker = jQuery('<em class="dc_mercury-marker"/>', @context).get(0)
    @endMarker = jQuery('<em class="dc_mercury-marker"/>', @context).get(0)
