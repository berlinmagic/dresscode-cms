...
class @DcMercury.Regions.Markupable extends DcMercury.Region
  ...
  build: ->
    ...
    @element.removeClass(DcMercury.config.regions.className)
    @textarea = jQuery('<textarea>', @document).val(value)
    @textarea.attr('class', @element.attr('class')).addClass('dc_mercury-textarea')
    ...
    @element.addClass(DcMercury.config.regions.className)
  ...
  bindEvents: ->
    ...
    DcMercury.on 'focus:frame', => @focus() if !@previewing && DcMercury.region == @

    DcMercury.on 'action', (event, options) =>
      return if @previewing || DcMercury.region != @
      @execCommand(options.action, options) if options.action

    DcMercury.on 'unfocus:regions', =>
      return if @previewing || DcMercury.region != @
      @element.blur()
  ...
  togglePreview: ->
    if @previewing
      @previewing = false
      @container.addClass(DcMercury.config.regions.className).removeClass("#{DcMercury.config.regions.className}-preview")
      @previewElement.hide()
      @element.show()
      @focus() if DcMercury.region == @
    else
      @previewing = true
      @container.addClass("#{DcMercury.config.regions.className}-preview").removeClass(DcMercury.config.regions.className)
  ...
  execCommand: (action, options = {}) ->
    ...
    handler.call(@, @selection(), options) if handler = DcMercury.Regions.Markupable.actions[action]
    @resize()
  ...
  selection: ->
    return new DcMercury.Regions.Markupable.Selection(@element)
  ...
  @actions: {
    ...
    formatblock: (selection, options) ->
      ...
      if options.value == 'blockquote'
        DcMercury.Regions.Markupable.actions.indent.call(@, selection, options)
        return
...
class DcMercury.Regions.Markupable.Selection
  ...
  placeMarker: ->
    @wrap('[dc_mercury-marker]', '[dc_mercury-marker]')


  removeMarker: ->
    val = @element.val()
    start = val.indexOf('[dc_mercury-marker]')
    return unless start > -1
    end = val.indexOf('[dc_mercury-marker]', start + 1) - '[dc_mercury-marker]'.length
    @element.val(@element.val().replace(/\[dc_mercury-marker\]/g, ''))
    @select(start, end)
