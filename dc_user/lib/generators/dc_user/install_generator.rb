      ...
      def build_seed_data
        Dir.glob( File.join(File.expand_path("#{DcUser::Engine.root}/db/system_data", __FILE__), "*.*") ) do |file|
          if File.exist?("#{Rails.root}/db/seeds.rb")
            ...
            append_to_file "#{Rails.root}/db/seeds.rb",  File.read( "#{DcUser::Engine.root}/db/system_data/#{File.basename(file.to_s)}"  )
          else
            puts("Info:: erstelle System-Seed .. ")
            create_file "#{Rails.root}/db/seeds.rb",  File.read(  "#{DcUser::Engine.root}/db/system_data/#{File.basename(file.to_s)}" )
          end
        end
      end
      ...
      def build_sample_data
        Dir.glob( File.join(File.expand_path("#{DcUser::Engine.root}/db/sample_data", __FILE__), "*.*") ) do |file|
          if File.exist?("#{Rails.root}/db/sample_seeds.rb")
            ...
            append_to_file "#{Rails.root}/db/sample_seeds.rb",  File.read( "#{DcUser::Engine.root}/db/sample_data/#{File.basename(file.to_s)}" )
          else
            puts("Info:: erstelle Sample-Seed ..")
            create_file "#{Rails.root}/db/sample_seeds.rb",  File.read( "#{DcUser::Engine.root}/db/sample_data/#{File.basename(file.to_s)}" )
          end
        end
      end
