    ...
    class InstallGenerator < ::Rails::Generators::Base
      ...
      def build_seed_data
        Dir.glob( File.join(File.expand_path("#{DcThemesDynamic::Engine.root}/db/system_data", __FILE__), "*.*") ) do |file|
          puts("DcThemesDynamic:: Seed system_data")
          open("#{Rails.root}/db/seeds.rb", 'a') do |f|
            f.puts File.read( "#{DcThemesDynamic::Engine.root}/db/system_data/#{File.basename(file.to_s)}"  ).to_utf
          end
        end
      end
      ...
      def build_sample_data
        Dir.glob( File.join(File.expand_path("#{DcThemesDynamic::Engine.root}/db/sample_data", __FILE__), "*.*") ) do |file|
          puts("DcThemesDynamic:: Seed sample_data")
          open("#{Rails.root}/db/sample_seeds.rb", 'a') do |f|
            f.puts File.read( "#{DcThemesDynamic::Engine.root}/db/sample_data/#{File.basename(file.to_s)}" ).to_utf
          end
        end
      end
