...
class @DcEditor.Dialog
  ...
  build: ->
    @element = jQuery('<div>', {class: "dc_editor-dialog dc_editor-#{@name}-dialog loading", style: 'display:none'})
  ...
  load: (callback) ->
    ...
    if DcEditor.preloadedViews[@url]
      ...
      @loadContent(DcEditor.preloadedViews[@url])
      ...
      DcEditor.dialogHandlers[@name].call(@) if DcEditor.dialogHandlers[@name]
      ...
      jQuery.ajax @url, {
        ...
        error: =>
          ...
          DcEditor.notify('DcEditor was unable to load %s for the "%s" dialog.', @url, @name)
      }
