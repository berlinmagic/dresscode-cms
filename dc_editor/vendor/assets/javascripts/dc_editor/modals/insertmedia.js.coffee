@DcEditor.modalHandlers.insertMedia = ->
  ...
  if DcEditor.region && DcEditor.region.selection
    selection = DcEditor.region.selection()
  ...
  @element.find('form').on 'submit', (event) =>
    ...
    switch type
      when 'image_url'
        ...
        DcEditor.trigger('action', {action: 'insertImage', value: attrs})

      when 'youtube_url'
        url = @element.find('#media_youtube_url').val()
        unless /^http:\/\/youtu.be\//.test(url)
          DcEditor.notify('Error: The provided youtube share url was invalid.')
          return
        ...
        DcEditor.trigger('action', {action: 'insertHTML', value: value})

      when 'vimeo_url'
        url = @element.find('#media_vimeo_url').val()
        unless /^http:\/\/vimeo.com\//.test(url)
          DcEditor.notify('Error: The provided vimeo url was invalid.')
          return
