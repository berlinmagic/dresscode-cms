@DcEditor.modalHandlers.insertLink = ->
  ...
  for link in jQuery('a[name]', window.dc_editorInstance.document)
  ...
  @element.find('form').on 'submit', (event) =>
    ...
    if existingLink
      DcEditor.trigger('action', {action: 'replaceLink', value: value, node: existingLink.get(0)})
    else
      DcEditor.trigger('action', {action: 'insertLink', value: value})
