...
## DcEditor general steps
#------------------------------------------------------------------------------
Given /^(?:|I )adjust the configuration to have: "([^"]*)"$/ do |javascript|
  Rails.application.config.dc_editor_config = javascript
end

# scoping step for the dc_editor content frame
When /^(.*) in the content frame$/ do |step|
  page.driver.within_frame('dc_editor_iframe') do
    When(step)
  end
end

# silence dc_editor's onbeforeunload confirmation
Given /^the editor won't prompt when leaving the page$/ do
  page.driver.execute_script('DcEditor.silent = true;')
end
  ...
  page.driver.within_frame('dc_editor_iframe') do
    ...
    page.driver.execute_script <<-JAVASCRIPT
      ...
      if (element.data('type') == 'markupable') {
        ...
        var region = top.dc_editorInstance.getRegionByName('#{region_id}');
  ...
  page.driver.within_frame('dc_editor_iframe') do
    ...
    page.driver.execute_script <<-JAVASCRIPT
      ...
      if (element.data('type') == 'markupable') {
        ...
        var selection = new top.DcEditor.Regions.Editable.Selection(window.getSelection(), document);
...
Given /^save results will be cached$/ do
  page.driver.execute_script <<-JAVASCRIPT
    DcEditor.PageEditor.prototype.oldSerialize = DcEditor.PageEditor.prototype.serialize;
    DcEditor.PageEditor.prototype.serialize = function() {
...
When /^(?:|I )(?:add|insert) a (row|column) (before|after)(?: it)?$/ do |row_or_column, before_or_after|
  ...
  When(%Q{I click on ".dc_editor-modal-content input[name=#{name}]"})
end
...
Then /^the(?: table)? (row|column) count should be (\d+)$/ do |row_or_column, expected_count|
  ...
  actual_count = page.driver.execute_script("return DcEditor.tableEditor.#{method}()")
...
Given /^the options for the (.*?) snippet "([^"]*)" are (.*?)$/ do |snippet_name, snippet_id, options|
  ...
  page.driver.execute_script <<-JAVASCRIPT
    DcEditor.Snippet.load({#{snippet_id}: {name: '#{snippet_name}', options: #{options_json}}});
  JAVASCRIPT
end
  ...
  page.driver.within_frame('dc_editor_iframe') do
    ...
    page.driver.execute_script <<-JAVASCRIPT
      ...
      if (element.data('type') == 'markupable') {
        ...
        document.execCommand('insertHTML', false, '<img data-snippet="#{snippet_name}" src="/assets/dc_editor/default-snippet.png">');
