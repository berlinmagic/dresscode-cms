describe "DcEditor.Toolbar.Button", ->

  template 'dc_editor/toolbar.button.html'

  beforeEach ->
    DcEditor.displayRect = {0, 0, 500, 200}
    DcEditor.Toolbar.Button.contexts.foo = -> true
    @region = {
      type: 'editable'
      element: $('<div class="dc_editor-region">')
      currentElement: -> $('<div>')
    }
    DcEditor.preloadedViews['/nothing'] = 'nothing'

  afterEach ->
    @button = null
    delete(@button)
    $(document).unbind('dc_editor:region:update')
    $(document).unbind('dc_editor:button')

  describe "constructor", ->

    it "expects name and title values", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title')
      html = $('<div>').html(@button).html()
      expect(html).toContain('title="title"')
      expect(html).toContain('class="dc_editor-button dc_editor-foo-button"')
      expect(html).toContain('<em>title</em>')

    it "accepts summary, types and options", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {palette: '/nothing'}, {appendDialogsTo: $('#test')})
      html = $('<div>').html(@button).html()
      expect(html).toContain('title="summary"')
      expect(html).toContain('class="dc_editor-button dc_editor-foo-button dc_editor-button-palette"')
      expect(html).toContain('<em>title</em>')

    it "calls build", ->
      spy = spyOn(DcEditor.Toolbar.Button.prototype, 'build').andCallFake(=>)
      spyOn(DcEditor.Toolbar.Button.prototype, 'bindEvents').andCallFake(=>)
      @button = new DcEditor.Toolbar.Button('foo', 'title')
      expect(spy.callCount).toEqual(1)

    it "calls bindEvents", ->
      spyOn(DcEditor.Toolbar.Button.prototype, 'build').andCallFake(=>)
      spy = spyOn(DcEditor.Toolbar.Button.prototype, 'bindEvents').andCallFake(=>)
      @button = new DcEditor.Toolbar.Button('foo', 'title')
      expect(spy.callCount).toEqual(1)
  ...
  describe "#build for various button types", ->
    ...
    it "attaches an element meant for the expander in button data", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title')
      expect(@button.data('expander')).toEqual('<div class="dc_editor-expander-button" data-button="foo"><em></em><span>title</span></div>')

    it "builds toggle buttons, that when clicked toggle their state", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {toggle: true})
    ...
    it "builds mode buttons, that when clicked fire a mode event", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {mode: true})
    ...
    it "builds buttons that understand context", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {context: true})
    ...
    it "builds panel buttons (and assigns toggle)", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {panel: '/evergreen/responses/blank.html'}, {appendDialogsTo: $('#test')})
      expect($('#test .dc_editor-panel').length).toEqual(1)
    ...
    it "builds palette buttons", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {palette: '/evergreen/responses/blank.html'}, {appendDialogsTo: $('#test')})
      expect($('#test .dc_editor-palette').length).toEqual(1)

    it "builds select buttons", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {select: '/evergreen/responses/blank.html'}, {appendDialogsTo: $('#test')})
      expect($('#test .dc_editor-select').length).toEqual(1)

    it "builds modal buttons", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {modal: '/evergreen/responses/blank.html'}, {appendDialogsTo: $('#test')})
      # nothing unique about this in building -- the modal is built/fired on click

    it "builds lightview buttons", ->
      @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {lightview: '/evergreen/responses/blank.html'}, {appendDialogsTo: $('#test')})
    ...
    it "throws an error when an unknown type is encountered", ->
      expect(=>
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {foo: '/evergreen/responses/blank.html'})
  ...
  describe "observed events", ->
    ...
    describe "custom event: button", ->
      ...
      it "calls click on the button itself", ->
        ...
        DcEditor.trigger('button', {action: 'foo'})
        expect(spy.callCount).toEqual(1)

    describe "custom event: mode", ->

      it "toggles a button when that mode is triggered and the button is toggleable", ->
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {mode: 'preview', toggle: true})
        spy = spyOn(DcEditor.Toolbar.Button.prototype, 'togglePressed').andCallFake(=>)
    ...
    describe "custom event: region:update", ->
      ...
      it "calls contexts if one is available and set", ->
        contextSpy = spyOn(DcEditor.Toolbar.Button.contexts, 'foo').andCallFake(-> true)
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {context: true})
    ...
    describe "custom event: region:focused", ->
      ...
      it "disables if the region type isn't supported", ->
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {context: true, regions: ['foo']})
      ...
      it "enables if the region type is supported", ->
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {context: true, regions: ['editable']})
    ...
    describe "click for various button types", ->
      ...
      it "does nothing if the button is disabled", ->
        spy = spyOn(DcEditor.Toolbar.Button.prototype, 'togglePressed').andCallThrough()
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {toggle: true})
      ...
      it "triggers an action event", ->
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary')
        spy = spyOn(DcEditor, 'trigger').andCallFake(=>)
      ...
      it "triggers a focus:frame event", ->
        spy = spyOn(DcEditor, 'trigger').andCallFake(=>)
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {}, {regions: ['editable']})
      ...
      it "opens a modal window", ->
        spy = spyOn(DcEditor, 'modal').andCallFake(=>)
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {modal: '/evergreen/responses/blank.html'})
      ...
      it "opens a lightview window", ->
        ...
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {lightview: '/evergreen/responses/blank.html'})
      ...
      it "shows and hides palettes", ->
        spy = spyOn(DcEditor.Palette.prototype, 'toggle').andCallFake(=>)
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {palette: '/evergreen/responses/blank.html'})
      ...
      it "shows and hides selects", ->
        spy = spyOn(DcEditor.Select.prototype, 'toggle').andCallFake(=>)
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {select: '/evergreen/responses/blank.html'})
      ...
      it "shows and hides panels, and toggles the button pressed state", ->
        spy = spyOn(DcEditor.Panel.prototype, 'toggle').andCallFake(=>)
        @button = new DcEditor.Toolbar.Button('foo', 'title', 'summary', {panel: '/evergreen/responses/blank.html'})
...
describe "DcEditor.Toolbar.Button.contexts", ->

  template 'dc_editor/toolbar.button.html'

  beforeEach ->
    @contexts = DcEditor.Toolbar.Button.contexts
