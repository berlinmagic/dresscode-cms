describe "DcEditor.tableEditor", ->

  template 'dc_editor/table_editor.html'
  ...
  describe "singleton method", ->
    ...
    beforeEach ->
      @loadSpy = spyOn(DcEditor.tableEditor, 'load').andCallFake(=>)
    ...
    it "returns the function object", ->
      ret = DcEditor.tableEditor(@table, @cell)
      expect(ret).toEqual(DcEditor.tableEditor)
  ...
  describe "#load", ->
    ...
    it "expects a table and a table cell", ->
      DcEditor.tableEditor(@table, @cell)
      expect(DcEditor.tableEditor.table).toEqual(@table)
      expect(DcEditor.tableEditor.cell).toEqual(@cell)

    it "accepts content that will be placed inside cells", ->
      DcEditor.tableEditor(@table, @cell, '<br/>')
      expect(DcEditor.tableEditor.cellContent).toEqual('<br/>')

    it "sets row based on where the cell is", ->
      DcEditor.tableEditor(@table, @cell)
      expect(DcEditor.tableEditor.row.get(0)).toEqual(@table.find('tr').get(0))

    it "gets the column count", ->
      DcEditor.tableEditor(@table, @cell)
      expect(DcEditor.tableEditor.columnCount).toEqual(6)

    it "gets the row count", ->
      DcEditor.tableEditor(@table, @cell)
      expect(DcEditor.tableEditor.rowCount).toEqual(8)
  ...
  describe "#cellIndexFor", ->
    ...
    it "gives the right index for different cells", ->
      ...
      expect(DcEditor.tableEditor.cellIndexFor(cell)).toEqual(0)

      cell = @table.find('#row2 th:nth-child(2n)').get(0)
      expect(DcEditor.tableEditor.cellIndexFor(cell)).toEqual(1)

      cell = @table.find('#row2 th:nth-child(6n)').get(0)
      expect(DcEditor.tableEditor.cellIndexFor(cell)).toEqual(5)

      cell = @table.find('#row3 td:nth-child(4n)').get(0)
      expect(DcEditor.tableEditor.cellIndexFor(cell)).toEqual(5)

      cell = @table.find('#row7 td:nth-child(2n)').get(0)
      expect(DcEditor.tableEditor.cellIndexFor(cell)).toEqual(2)
  ...
  describe "#cellSignatureFor", ->
    ...
    it "returns an object with cell information", ->
      ...
      sig = DcEditor.tableEditor.cellSignatureFor(cell)
  ...
  describe "#findCellByOptionsFor", ->
    ...
    it "finds a matching cell in a specific row, based on right", ->
      ...
      sig = DcEditor.tableEditor.findCellByOptionsFor(row, { right: 5 })
    ...
    it "finds a cell based on left", ->
      ...
      sig = DcEditor.tableEditor.findCellByOptionsFor(row, { left: 5 })
    ...
    it "finds a cell based on left and width", ->
      ...
      sig = DcEditor.tableEditor.findCellByOptionsFor(row, { left: 2, width: 4 })
    ...
    describe "when a cell isn't there", ->
      ...
      it "returns null", ->
        sig = DcEditor.tableEditor.findCellByOptionsFor(@row, { left: 2 })
        expect(sig).toEqual(null)

      it "can force to an adjacent cell", ->
        sig = DcEditor.tableEditor.findCellByOptionsFor(@row, { left: 2, forceAdjacent: true })
  ...
  describe "#findCellByIntersectionFor", ->
    ...
    it "finds cells that intersect vertically, based on the signature of another cell", ->
      sig = DcEditor.tableEditor.cellSignatureFor(@table.find('#row6 td:nth-child(3n)'))
      intersectingSig = DcEditor.tableEditor.findCellByIntersectionFor(@table.find('#row7'), sig)
  ...
  describe "#columnsFor", ->
    ...
    it "returns the total number of cells and colspans for a given collection of cells", ->
      expect(DcEditor.tableEditor.columnsFor(@table.find('#row7 td'))).toEqual(6)
  ...
  describe "#colspanFor", ->
    ...
    it "returns the colspan of a given cell", ->
      ...
      expect(DcEditor.tableEditor.colspanFor(cell)).toEqual(4)

    it "defaults to 1", ->
      cell = @table.find('#row6 td:first-child')
      expect(DcEditor.tableEditor.colspanFor(cell)).toEqual(1)
  ...
  describe "#rowspanFor", ->
    ...
    it "returns the rowspan of a given cell", ->
      ...
      expect(DcEditor.tableEditor.rowspanFor(cell)).toEqual(4)

    it "defaults to 1", ->
      cell = @table.find('#row6 td:first-child')
      expect(DcEditor.tableEditor.rowspanFor(cell)).toEqual(1)
  ...
  describe "setColspanFor", ->
    ...
    it "sets the colspan for a cell", ->
      DcEditor.tableEditor.setColspanFor(@cell, 20)
      expect(@cell.attr('colspan')).toEqual('20')

    it "removes the attribute if it's 1", ->
      DcEditor.tableEditor.setColspanFor(@cell, 1)
  ...
  describe "setRowspanFor", ->
    ...
    it "sets the rowspan for a cell", ->
      DcEditor.tableEditor.setRowspanFor(@cell, 20)
      expect(@cell.attr('rowspan')).toEqual('20')

    it "removes the attribute if it's 1", ->
      DcEditor.tableEditor.setRowspanFor(@cell, 1)
