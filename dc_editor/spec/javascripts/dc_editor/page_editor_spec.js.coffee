describe "DcEditor.PageEditor", ->

  template 'dc_editor/page_editor.html'
  ...
  afterEach ->
    @pageEditor = null
    delete(@pageEditor)
    window.dc_editorInstance = null
    $(window).unbind('dc_editor:initialize:frame')
    $(window).unbind('dc_editor:focus:frame')
    $(window).unbind('dc_editor:focus:window')
    $(window).unbind('dc_editor:toggle:interface')
    $(window).unbind('dc_editor:reinitialize')
  ...
  describe "constructor", ->
    ...
    beforeEach ->
      @initializeInterfaceSpy = spyOn(DcEditor.PageEditor.prototype, 'initializeInterface').andCallFake(=>)

    it "throws an error if it's not supported", ->
      DcEditor.supported = false
      expect(=>
        new DcEditor.PageEditor()
      ).toThrow('DcEditor.PageEditor is unsupported in this client. Supported browsers are chrome 10+, firefix 4+, and safari 5+.')
      DcEditor.supported = true

    it "throws an error if it's already instantiated", ->
      window.dc_editorInstance = true
      expect(=>
        new DcEditor.PageEditor()
      ).toThrow('DcEditor.PageEditor can only be instantiated once.')
      window.dc_editorInstance = false

    it "sets the dc_editorInstance to window", ->
      @pageEditor = new DcEditor.PageEditor()
      expect(window.dc_editorInstance).toEqual(@pageEditor)

    it "accepts a saveUrl, and options", ->
      @pageEditor = new DcEditor.PageEditor('/foo/1', {foo: 'bar'})
    ...
    it "sets the visible option to true unless it's set", ->
      @pageEditor = new DcEditor.PageEditor('/foo/1', {foo: 'bar', visible: false})
      expect(@pageEditor.options.visible).toEqual(false)
      window.dc_editorInstance = null
      @pageEditor = new DcEditor.PageEditor('/foo/1', {foo: 'bar', visible: true})
    ...
    it "gets the csrf token if there's one available", ->
      new DcEditor.PageEditor()
      expect(DcEditor.csrfToken).toEqual('K6JhyfOVKJX8X2ZkiJXSf491fc1fF+k79wzrChHQa0g=')


  describe "#initializeInterface", ->

    beforeEach ->
      @resizeSpy = spyOn(DcEditor.PageEditor.prototype, 'resize').andCallFake(=>)
      DcEditor.Toolbar = -> {toolbar: true}
      DcEditor.Statusbar = -> {statusbar: true}
      @iframeSrcSpy = spyOn(DcEditor.PageEditor.prototype, 'iframeSrc').andCallFake(=> '/foo')

    it "builds a focusable element (so we can get focus off the iframe)", ->
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})
      expect($('input.dc_editor-focusable[type=text]').length).toEqual(1)

    it "builds an iframe", ->
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})
      expect($('iframe.dc_editor-iframe').length).toEqual(1)

    it "appends the elements to any node", ->
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#page_editor_container')})
      expect($('#page_editor_container input[type=text]').length).toEqual(1)
      expect($('#page_editor_container iframe.dc_editor-iframe').length).toEqual(1)
  ...
  describe "#initializeFrame", ->
    ...
    beforeEach ->
      @resizeSpy = spyOn(DcEditor.PageEditor.prototype, 'resize').andCallFake(=>)
      @bindEventsSpy = spyOn(DcEditor.PageEditor.prototype, 'bindEvents').andCallFake(=>)
      @initializeRegionsSpy = spyOn(DcEditor.PageEditor.prototype, 'initializeRegions').andCallFake(=>)
      @finalizeInterfaceSpy = spyOn(DcEditor.PageEditor.prototype, 'finalizeInterface')
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})
    ...
    it "injects needed dc_editor styles", ->
      @finalizeInterfaceSpy.andCallFake(=>)
      spy = spyOn($.fn, 'appendTo').andCallFake(=>)
      @pageEditor.initializeFrame()
      expect(spy.callCount).toEqual(1)

    it "injects dc_editor namespace into the iframe", ->
      @finalizeInterfaceSpy.andCallFake(=>)
      @pageEditor.initializeFrame()
      expect(@pageEditor.iframe.get(0).contentWindow.DcEditor).toEqual(window.DcEditor)
    ...
    it "fires the ready event (DcEditor.trigger)", ->
    ...
    it "fires the ready event (jQuery.trigger)", ->
      ...
      expect(spy.argsForCall[0]).toEqual(['dc_editor:ready', undefined])
    ...
    it "fires the ready event (Event.fire)", ->
      ...
      @pageEditor.initializeFrame()
      expect(spy.callCount).toEqual(1)
      expect(spy.argsForCall[0][1]).toEqual('dc_editor:ready')

    it "calls onDcEditorReady in the iframe", ->
      @finalizeInterfaceSpy.andCallFake(=>)
      iframeWindow = @pageEditor.iframe.get(0).contentWindow
      iframeWindow.onDcEditorReady = ->
      spy = spyOn(iframeWindow, 'onDcEditorReady').andCallFake(=>)
      @pageEditor.initializeFrame()
      expect(spy.callCount).toEqual(1)
    ...
    it "captures errors and alerts them", ->
      ...
      @pageEditor.initializeFrame()
      expect(spy.callCount).toEqual(1)
      expect(spy.argsForCall[0]).toEqual(['DcEditor.PageEditor failed to load: unknown error\n\nPlease try refreshing.'])
  ...
  describe "#initializeRegions", ->
    ...
    beforeEach ->
      @resizeSpy = spyOn(DcEditor.PageEditor.prototype, 'resize').andCallFake(=>)
      DcEditor.PageEditor.prototype.initializeFrame = ->
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})
      @pageEditor.document = $(document)

    it "it calls buildRegion for all the regions found in a document", ->
      spy = spyOn(DcEditor.PageEditor.prototype, 'buildRegion').andCallFake(=>)
  ...
  describe "#buildRegion", ->
    ...
    beforeEach ->
      @resizeSpy = spyOn(DcEditor.PageEditor.prototype, 'resize').andCallFake(=>)
      DcEditor.PageEditor.prototype.initializeFrame = ->
      DcEditor.Regions.Editable = -> {region: true}
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})
    ...
    it "alerts on errors", ->
      ...
      DcEditor.Regions.Editable = -> throw('error!')
      spy = spyOn(window, 'alert').andCallFake(=>)
      @pageEditor.buildRegion($('#region2'))
      expect(spy.callCount).toEqual(1)
    ...
    it "calls togglePreview on the region if in preview mode", ->
      callCount = 0
      DcEditor.Regions.Editable = -> {region: true, togglePreview: -> callCount += 1 }
  ...
  describe "#finalizeInterface", ->
    ...
    beforeEach ->
      @resizeSpy = spyOn(DcEditor.PageEditor.prototype, 'resize').andCallFake(=>)
      DcEditor.PageEditor.prototype.initializeFrame = ->
      DcEditor.SnippetToolbar = -> {snippetToolbar: true}
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})
      @highjackLinksAndFormsSpy = spyOn(DcEditor.PageEditor.prototype, 'hijackLinksAndForms').andCallFake(=>)

    it "injects an sanitizing element used for sanitizing content", ->
      @pageEditor.finalizeInterface()
      expect($('#dc_editor_sanitizer[contenteditable]').length).toEqual(1)
  ...
  describe "observed events", ->
    ...
    describe "custom event: initialize:frame", ->
      ...
      it "calls initializeFrame", ->
        @initializeFrameSpy = spyOn(DcEditor.PageEditor.prototype, 'initializeFrame').andCallFake(=>)
        @setTimeoutSpy = spyOn(window, 'setTimeout').andCallFake((timeout, callback) -> callback())
        DcEditor.trigger('initialize:frame')
    ...
    describe "custom event: toggle:interface", ->
      ...
      it "calls toggleInterface", ->
        spy = spyOn(DcEditor.PageEditor.prototype, 'toggleInterface').andCallFake(=>)
        DcEditor.trigger('toggle:interface')
        expect(spy.callCount).toEqual(1)
    ...
    describe "custom event: reinitialize", ->
      ...
      it "calls initializeRegions", ->
        spy = spyOn(DcEditor.PageEditor.prototype, 'initializeRegions').andCallFake(=>)
    ...
    describe "custom event: action", ->
      ...
      it "calls save if the action was save", ->
        spy = spyOn(DcEditor.PageEditor.prototype, 'save').andCallFake(=>)
        DcEditor.trigger('action', {action: 'foo'})
        expect(spy.callCount).toEqual(0)

        DcEditor.trigger('action', {action: 'save'})
        expect(spy.callCount).toEqual(1)
    ...
    describe "mousedown on document", ->
      ...
      it "triggers hide:dialogs", ->
        DcEditor.region = {element: $('#region3')}
      ...
      it "triggers unfocus:regions unless the event happened in a region", ->
        DcEditor.region = {element: {get: -> null}}
    ...
    describe "window resize", ->
      ...
      # untestable
      it "calls resize", ->
        #spy = spyOn(DcEditor.PageEditor.prototype, 'resize').andCallFake(=>)
    ...
    describe "onbeforeunload", ->
      ...
      # untestable
      it "calls DcEditor.beforeUnload", ->
        #spy = spyOn(DcEditor, 'beforeUnload').andCallFake(=>)
  ...
  describe "#toggleInterface", ->
    ...
    beforeEach ->
      ...
      DcEditor.Toolbar = -> {toolbar: true, height: (-> 100), show: (=> spec.toolbarShowCallCount += 1), hide: (=> spec.toolbarHideCallCount += 1)}
      DcEditor.Statusbar = -> {statusbar: true, top: (-> 500), show: (=> spec.statusbarShowCallCount += 1), hide: (=> spec.statusbarHideCallCount += 1)}
      DcEditor.PageEditor.prototype.initializeFrame = ->
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})
  ...
  describe "#resize", ->
    ...
    beforeEach ->
      DcEditor.Toolbar = -> {toolbar: true, height: -> 100}
      DcEditor.Statusbar = -> {statusbar: true, top: -> 500}
      DcEditor.PageEditor.prototype.initializeFrame = ->
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})

    it "sets the display rectangle to displayRect", ->
      @pageEditor.resize()
      expect(DcEditor.displayRect.top).toEqual(100)
      expect(DcEditor.displayRect.height).toEqual(500 - 100)

    it "resizes the iframe", ->
      @pageEditor.resize()
      expect($('.dc_editor-iframe').css('height')).toEqual("#{500 - 100}px")
  ...
  describe "#hijackLinksAndForms", ->
    ...
    beforeEach ->
      DcEditor.config.nonHijackableClasses = ['lightview']
    ...
    afterEach ->
      DcEditor.config.nonHijackableClasses = []
  ...
  describe "#beforeUnload", ->
    ...
    beforeEach ->
      DcEditor.PageEditor.prototype.initializeInterface = ->
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})
      DcEditor.silent = false
  ...
  describe "#getRegionByName", ->
    ...
    beforeEach ->
      DcEditor.PageEditor.prototype.initializeInterface = ->
      @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test')})
      @iframeSrcSpy = spyOn(DcEditor.PageEditor.prototype, 'iframeSrc').andCallFake(=> '/foo/baz')
      @ajaxSpy = spyOn($, 'ajax')
  ...
  describe "#save", ->
    ...
    describe "POST", ->
      beforeEach ->
        DcEditor.PageEditor.prototype.initializeInterface = ->
        @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test'), saveDataType: 'text'})
        @iframeSrcSpy = spyOn(DcEditor.PageEditor.prototype, 'iframeSrc').andCallFake(=> '/foo/baz')
        @ajaxSpy = spyOn($, 'ajax')

      it "doesn't set the _method in the request data", ->
        @ajaxSpy.andCallFake(=>)
        spyOn(DcEditor.PageEditor.prototype, 'serialize').andCallFake(=> {region1: 'region1'})
        @pageEditor.save()
      ...
      it "sets headers by calling DcEditor.ajaxHeaders", ->
        @ajaxSpy.andCallFake(=>)
        spyOn(DcEditor.PageEditor.prototype, 'serialize').andCallFake(=> {region1: 'region1'})
      ...
      describe "on successful ajax request", ->
        ...
        it "sets changes back to false", ->
          DcEditor.changes = true
          @pageEditor.save()
          expect(DcEditor.changes).toEqual(false)
      ...
      describe "on failed ajax request", ->
        ...
        it "alerts with the url", ->
          ...
          @pageEditor.save()
          expect(spy.callCount).toEqual(1)
          expect(spy.argsForCall[0]).toEqual(['DcEditor was unable to save to the url: /foo/bar'])

    describe "PUT", ->

      beforeEach ->
        DcEditor.PageEditor.prototype.initializeInterface = ->
        @pageEditor = new DcEditor.PageEditor('', {appendTo: $('#test'), saveMethod: 'PUT'})
        @iframeSrcSpy = spyOn(DcEditor.PageEditor.prototype, 'iframeSrc').andCallFake(=> '/foo/baz')
        @ajaxSpy = spyOn($, 'ajax')
