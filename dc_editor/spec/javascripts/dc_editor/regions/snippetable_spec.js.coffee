describe "DcEditor.Regions.Snippetable", ->

  template 'dc_editor/regions/snippetable.html'
  ...
  describe "constructor", ->
    ...
    beforeEach ->
      @buildSpy = spyOn(DcEditor.Regions.Snippetable.prototype, 'build').andCallFake(=>)
      @bindEventsSpy = spyOn(DcEditor.Regions.Snippetable.prototype, 'bindEvents').andCallFake(=>)
      @makeSortableSpy = spyOn(DcEditor.Regions.Snippetable.prototype, 'makeSortable').andCallFake(=>)

    it "expects an element and window", ->
      @region = new DcEditor.Regions.Snippetable(@regionElement, window)
      expect(@region.element.get(0)).toEqual($('#snippetable_region1').get(0))
      expect(@region.window).toEqual(window)

    it "accepts options", ->
      @region = new DcEditor.Regions.Snippetable(@regionElement, window, {foo: 'something'})
  ...
  describe "#build", ->
    ...
    beforeEach ->
      spyOn(DcEditor.Regions.Snippetable.prototype, 'bindEvents').andCallFake(=>)
  ...
  describe "observed events", ->
    ...
    describe "custom event: unfocus:regions", ->
      ...
      it "removes the focus class", ->
        ...
        DcEditor.trigger('unfocus:regions')
      ...
      it "triggers the region:blurred event", ->
        spy = spyOn(DcEditor, 'trigger').andCallThrough()
        DcEditor.trigger('unfocus:regions')
    ...
    describe "keydown on document (for undo / redo)", ->
      ...
      it "calls execCommand with undo on meta+z", ->
        spy = spyOn(DcEditor.Regions.Snippetable.prototype, 'execCommand')
    ...
    describe "mouseup", ->
      ...
      it "calls focus", ->
        spy = spyOn(DcEditor.Regions.Snippetable.prototype, 'focus')
  ...
  describe "#focus", ->
    ...
    it "sets the active dc_editor region", ->
      DcEditor.region = null
      @region.focus()
      expect(DcEditor.region).toEqual(@region)

    it "makes the snippets sortable again", ->
      spy = spyOn(DcEditor.Regions.Snippetable.prototype, 'makeSortable')
      @region.focus()
      expect(spy.callCount).toEqual(1)
  ...
  describe "#execCommand", ->
    ...
    beforeEach ->
      @region = new DcEditor.Regions.Snippetable(@regionElement, window)
      DcEditor.Regions.Snippetable.actions['foo'] = ->
      @handlerSpy = spyOn(DcEditor.Regions.Snippetable.actions, 'foo')
  ...
  describe "#makeSortable", ->
    ...
    it "pushes to the history after dragging", ->
      spy = spyOn(DcEditor.Regions.Snippetable.prototype, 'pushHistory').andCallFake(=>)
...
describe "DcEditor.Regions.Snippetable.actions", ->

  template 'dc_editor/regions/snippetable.html'

  beforeEach ->
    @region = new DcEditor.Regions.Snippetable($('#snippetable_region2'), window)
    @actions = DcEditor.Regions.Snippetable.actions

  describe ".undo", ->

    it "calls undo on the history buffer and sets the content", ->
      htmlSpy = spyOn(DcEditor.Regions.Snippetable.prototype, 'content').andCallFake(=>)
  ...
  describe ".insertSnippet", ->
    ...
    beforeEach ->
      ...
      spyOn(DcEditor.Snippet.prototype, 'loadPreview').andCallFake(=>)

    describe "updating a snippet", ->

      it "finds the snippet by it's identity and replaces it with the new snippet", ->
        @actions['insertSnippet'].call(@region, {value: DcEditor.Snippet.find('snippet_1')})
        expect($('#snippetable_region2').html()).toContain('class="dc_editor-snippet"')
      ...
      it "pushes to the history after it's been rendered", ->
        spyOn(DcEditor.Snippet.prototype, 'getHTML').andCallFake((x, callback) => callback() if callback)
        spy = spyOn(DcEditor.Regions.Snippetable.prototype, 'pushHistory').andCallFake(=>)
        @actions['insertSnippet'].call(@region, {value: DcEditor.Snippet.find('snippet_1')})
        expect(spy.callCount).toEqual(1)
    ...
    describe "inserting a snippet", ->
      ...
      it "appends the new snippet html to the element", ->
        @actions['insertSnippet'].call(@region, {value: DcEditor.Snippet.find('snippet_2')})
        expect($('#snippetable_region2 .dc_editor-snippet').length).toEqual(2)
  ...
  describe ".editSnippet", ->
    ...
    beforeEach ->
      @region.snippet = $('#snippetable_region2 .dc_editor-snippet')

    it "finds and displays the options for the given snippet", ->
      spy = spyOn(DcEditor.Snippet.prototype, 'displayOptions')
  ...
  describe ".removeSnippet", ->
    ...
    it "removes the snippet if there's an active one", ->
      ...
      expect($('#snippetable_region2 .dc_editor-snippet').length).toEqual(0)
