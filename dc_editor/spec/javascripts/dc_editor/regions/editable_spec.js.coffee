describe "DcEditor.Regions.Editable", ->

  template 'dc_editor/regions/editable.html'
  ...
  describe "constructor", ->
     ...
     beforeEach ->
       @buildSpy = spyOn(DcEditor.Regions.Editable.prototype, 'build').andCallFake(=>)
       @bindEventsSpy = spyOn(DcEditor.Regions.Editable.prototype, 'bindEvents').andCallFake(=>)

     it "expects an element and window", ->
       @region = new DcEditor.Regions.Editable(@regionElement, window)
       expect(@region.element.get(0)).toEqual($('#editable_region1').get(0))
       expect(@region.window).toEqual(window)

     it "accepts options", ->
       @region = new DcEditor.Regions.Editable(@regionElement, window, {foo: 'something'})
  ...
  describe "#build", ->
    ...
    it "sets dc_editorEditing on the document", ->
  ...
  describe "observed events", ->
    ...
    describe "paste", ->
      ...
      it "tells dc_editor that changes have been made", ->
  ...
  describe "#execCommand", ->
    ...
    describe "when a handler exists", ->
      ...
      it "calls it if one is defined in DcEditor.config.behaviors", ->
...
describe "DcEditor.Regions.Editable.actions", ->

  template 'dc_editor/regions/editable.html'

  beforeEach ->
    #@region = new DcEditor.Regions.Editable($('#editable_region1'), window)
    #@actions = DcEditor.Regions.Editable.actions
