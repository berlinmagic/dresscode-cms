describe "DcEditor.Regions.Markupable", ->

  template 'dc_editor/regions/markupable.html'
  ...
  describe "constructor", ->
     ...
     beforeEach ->
       @buildSpy = spyOn(DcEditor.Regions.Markupable.prototype, 'build').andCallFake(=>)
       @bindEventsSpy = spyOn(DcEditor.Regions.Markupable.prototype, 'bindEvents').andCallFake(=>)
       spyOn(DcEditor.Regions.Markupable.prototype, 'pushHistory').andCallFake(=>)

     it "expects an element and window", ->
       @region = new DcEditor.Regions.Markupable(@regionElement, window)
       expect(@region.element.get(0)).toEqual($('#markupable_region1').get(0))
       expect(@region.window).toEqual(window)

     it "accepts options", ->
       @region = new DcEditor.Regions.Markupable(@regionElement, window, {foo: 'something'})
...
describe "DcEditor.Regions.Markupable.actions", ->

  template 'dc_editor/regions/markupable.html'

  beforeEach ->
    #@region = new DcEditor.Regions.Markupable($('#markupable_region1'), window)
    #@actions = DcEditor.Regions.Markupable.actions
