class @DcEditor.Regions.Editable extends DcEditor.Region
  type = 'editable'
  ...
  build: ->
    ...
    for element in @element.find('.dc_editor-snippet')
      element.contentEditable = false
      jQuery(element).attr('data-version', '1')

    # add the basic editor settings to the document (only once)
    unless @document.dc_editorEditing
      @document.dc_editorEditing = true
      try
  ...
  bindEvents: ->
    ...
    DcEditor.on 'region:update', =>
      ...
      if currentElement.length
        ...
        DcEditor.tableEditor(table, currentElement.closest('tr, td'), '<br/>') if table.length
        # display a tooltip if we're in an anchor
        anchor = currentElement.closest('a', @element)
        if anchor.length && anchor.attr('href')
          DcEditor.tooltip(anchor, "<a href=\"#{anchor.attr('href')}\" target=\"_blank\">#{anchor.attr('href')}</a>", {position: 'below'})
        else
          DcEditor.tooltip.hide()

    @element.on 'dragenter', (event) =>
      return if @previewing
      event.preventDefault() unless DcEditor.snippet
    ...
    @element.on 'possible:drop', =>
      ...
      if snippet = @element.find('img[data-snippet]').get(0)
        @focus()
        DcEditor.Snippet.displayOptionsFor(jQuery(snippet).data('snippet'))
    ...
    @element.on 'dblclick', (event) =>
      ...
      if image.length
        ...
        DcEditor.trigger('button', {action: 'insertMedia'})
  ...
  content: (value = null, filterSnippets = true, includeMarker = false) ->
    if value != null
      ...
      for element in container.find('[data-snippet]')
        ...
        if snippet = DcEditor.Snippet.find(element.data('snippet'))
  ...
  execCommand: (action, options = {}) ->
    ...
    if handler = DcEditor.config.behaviors[action] || DcEditor.Regions.Editable.actions[action]
    ...
    @element.find('img').one 'error', -> jQuery(@).attr({src: '/assets/dc_editor/missing-image.png', title: 'Image not found'})
  ...
  selection: ->
    return new DcEditor.Regions.Editable.Selection(@window.getSelection(), @document)
  ...
  handlePaste: (event) ->
    ...
    if DcEditor.config.pasting.sanitize == 'text' && event.clipboardData
      ...
      sanitizer = jQuery('#dc_editor_sanitizer', @document).focus()
  ...
  sanitize: (sanitizer) ->
    ...
    sanitizer.find(".#{DcEditor.config.regionClass}").remove()

    if DcEditor.config.pasting.sanitize
      switch DcEditor.config.pasting.sanitize
        when 'blacklist'
        ...
        when 'whitelist'
          for element in sanitizer.find('*')
            allowed = false
            for allowedTag, allowedAttributes of DcEditor.config.pasting.whitelist
  ...
  @actions: {
    ...
    insertRowBefore: -> DcEditor.tableEditor.addRow('before')

    insertRowAfter: -> DcEditor.tableEditor.addRow('after')

    insertColumnBefore: -> DcEditor.tableEditor.addColumn('before')

    insertColumnAfter: -> DcEditor.tableEditor.addColumn('after')

    deleteColumn: -> DcEditor.tableEditor.removeColumn()

    deleteRow: -> DcEditor.tableEditor.removeRow()

    increaseColspan: -> DcEditor.tableEditor.increaseColspan()

    decreaseColspan: -> DcEditor.tableEditor.decreaseColspan()

    increaseRowspan: -> DcEditor.tableEditor.increaseRowspan()

    decreaseRowspan: -> DcEditor.tableEditor.decreaseRowspan()
...
class DcEditor.Regions.Editable.Selection
  ...
  forceSelection: (element) ->
    ...
    if @range
      if @commonAncestor(true).closest('.dc_editor-snippet').length
  ...
  selectMarker: (context) ->
    markers = context.find('em.dc_editor-marker')
  ...
  placeMarker: ->
    ...
    @startMarker = jQuery('<em class="dc_editor-marker"/>', @context).get(0)
    @endMarker = jQuery('<em class="dc_editor-marker"/>', @context).get(0)
